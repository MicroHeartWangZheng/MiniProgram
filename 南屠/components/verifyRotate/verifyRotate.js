  ...
  n = {
    ...
    data: function() {
      ...
      return t(i = {
        ...
        initRotateZ: 0,
        questionInfo: {
          idePicture: "https://jdmqn.19371213.com.cn/jdm/miniAppImages/1.jpg",
          diffL: 2,
          diffR: 2,
          correctOption: {
            originalWidth: 200,
    ...
    computed: {
      rotateZ: function() {
        return Math.round(this.nowX * this.randomIntInRange)
      }
    },
    unmounted: function() {
    ...
    methods: {
      ...
      confirm: function() {
        var t = this.rotateZ + this.initRotateZ,
          i = !1;
        ((t %= 360) - this.questionInfo.diffL <= 0 || t + this.questionInfo.diffR >= 360) && (i = !0), i ? (this.isRight = !0, this.$emit("afterAnswer")) : this.isRight = !1
      },
      init: function() {
        var t = Math.round(14 * Math.random() + 1),
          n = this.$settings.verifyImg + "common/" + t + ".jpg";
        this.$set(this.questionInfo, "idePicture", n);
        var o = this.questionInfo.correctOption,
          e = o.originalWidth,
          s = e / 554 * 100,
          r = e / 368 * 100,
          h = Math.round(Math.random() * (100 - 2 * r) + .5 * r),
          a = Math.round(Math.random() * (100 - 2 * s) + .5 * s);
        o.top = h, o.left = a;
        var l = 554,
          u = i.wx$1.getSystemInfoSync(),
          f = .7386 * u.windowWidth / l;
        this.windowInfo = u;
        var d = o.originalWidth / l * 100,
          g = o.originalHeight / 368 * 100,
          c = this.questionInfo.diffL / (l * f) * 100,
          p = this.questionInfo.diffR / (l * f) * 100;
        g + o.top > 100 && (o.top = 100 - g);
        var m = {
          width: "1000px",
          height: "1000px",
          top: o.top + "%",
          left: o.left + "%",
          transform: "scale(" + Math.round(f * l * d / 100) / 1e3 + ")"
        };
        this.maxLeft = Math.round(.7386 * u.windowWidth * 85.5595667870036 / 100);
        var w = {
            width: Math.round(f * l * d / 100) + "px",
            height: Math.round(f * l * d / 100) + "px",
            top: o.top + "%",
            left: "10",
            scale: Math.round(f * l * d / 100) / 1e3
          },
          I = {
            left: 0 - o.left * (100 / d) + "%",
            top: 0 - o.top * (100 / g) + "%",
            width: 1 / (Math.round(f * l * d / 100) / 1e3) * 554 + "rpx",
            height: 1 / (Math.round(f * l * d / 100) / 1e3) * 368 + "rpx"
          };
        this.sLeft = o.left, this.diffL = c, this.diffR = p, this.sliderStyle = m, this.sliderImgStyle = w, this.imgStyle = I, this.isInit = !0;
        var y = Math.floor(341 * Math.random()) + 10;
        this.randomIntInRange = 360 / this.maxLeft, this.initRotateZ = y
      }
    }
  };
Array || i.resolveComponent("uni-icons")(), Math;
var o = i._export_sfc(n, [
  ["render", function(t, n, o, e, s, r) {
    return i.e({
      a: !1 === s.isRight
    }, (s.isRight, {}), {
      b: !0 === s.isRight
    }, (s.isRight, {}), {
      c: s.questionInfo && s.questionInfo.idePicture && s.isInit
    }, s.questionInfo && s.questionInfo.idePicture && s.isInit ? i.e({
      d: s.questionInfo.idePicture,
      e: 1 == s.isRight ? 1 : "",
      f: i.s(s.sliderStyle),
      g: s.questionInfo.idePicture,
      h: i.s(s.imgStyle),
      i: -1 == s.isRight
    }, -1 == s.isRight ? {
      j: s.failImg
      ...
      n: " scale(".concat(s.sliderImgStyle.scale, ")"),
      o: "translate(-50%,-50%) rotateZ(".concat(r.rotateZ + s.initRotateZ, "deg)"),
      p: s.sliderImgStyle.top,
      q: s.sliderStyle.left,
      r: s.sliderImgStyle.width,
      s: s.sliderImgStyle.width,
      t: "url('".concat(s.slideComBg, "')")
    }) : {}, {
