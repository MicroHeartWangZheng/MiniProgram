...
Page({
  ...
  codeBtnClick: function(e) {
    ...
    if (console.log("vcode", this.data.vcode), 1 != this.data.showCountdown)
      if (this.setData({
        ...
        }), "" == this.data.phoneNum) wx.showToast({
        title: "请先输入手机号码",
        icon: "none"
      });
  ...
  loginClick: function(e) {
    if ("" == this.data.phoneNum.replace(/\s+/g, "")) wx.showToast({
      ...
      if ("" == this.data.vcode) wx.showToast({
        title: "请输入图形验证码",
        icon: "none"
      });
      ...
      wx.login({
        success: function(e) {
          e.code && (0, t.loginPhone)(n, i, e.code).then((function(t) {
            wx.hideLoading(), 200 === t.code ? (getApp().globalData.authorizationc = t.data.authorizationc, (0, o.toast)("登录成功"), setTimeout((function() {
              wx.navigateBack()
